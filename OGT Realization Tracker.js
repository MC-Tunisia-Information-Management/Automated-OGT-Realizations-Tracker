...
function dataManipulation(dataSet, sheet) {
  ...
  for (var i = 0; i < dataSet.length; i++) {
    ...
    var uniqueBackgroundsString = uniqueBackgrounds.join(", ");
    ...
    if (rowIndex.length == 0) {
      ...
      if (dataSet[i].status == "realized") {
        ...
        if (
          dataSet[i].date_realized != null &&
          dataSet[i].date_realized.length > 0
        ) {
          // Choose the first date if multiple dates are present
          row[0][23] = dataSet[i].date_realized[0].toString().substring(0, 10);
        } else {
          // Handle the case when no date is present
          row[0][23] = "-";
        }
        if (dataSet[i].experience_end_date != null) {
          row[0][24] = dataSet[i].experience_end_date
...
function main() {
  ...
  var updatingSheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName("OGTa/e");
